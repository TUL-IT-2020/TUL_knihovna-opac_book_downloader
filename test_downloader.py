...
import pytest
import book_downloader as TUL_dwnldr
...
def test_parse_file_number_from_url(url, file_number):
    assert TUL_dwnldr.parse_file_number_from_url(url) == file_number
...
def test_parse_book_number_from_url(url, book_number):
    assert TUL_dwnldr.parse_book_number_from_url(url) == book_number